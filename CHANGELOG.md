...
- 导入与 GitHub 向导提供来源扫描、候选选择、冲突策略、固定 commit、目标 CLI/同步方式、取消和逐项结果；部分失败会保留源包并显示失败目标。MCP 原生配置在本任务提供投影预览，未自动整文件改写原生配置。

### MCP/Skills 项目与 Worktree 策略

- 项目和 Worktree 新增独立的 MCP/Skills 策略编辑入口；每个 CLI 分别支持继承或自定义集合，预览实际全局状态、有效集合、启动时应用集合和失效选项，取消不会写入策略。
- 新启动和创建新进程的恢复按项目/Worktree、CLI、环境和策略 revision 生成受管快照：Claude 使用 `--mcp-config`/`--strict-mcp-config` 与保留原设置的 `skillOverrides`，Codex 使用 `-c` 与路径级 `skills.config`；不改写原生项目/全局配置。
