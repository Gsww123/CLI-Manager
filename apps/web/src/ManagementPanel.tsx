        ...
        <table className="management-table">
          ...
          <tbody>
            {entries.map((entry, index) => {
              ...
              return (
                <tr key={`${path || index}-${index}`}>
                  <td>
                    {onSelectPath && path ? (
                      <button
                        className="result-path-button"
                        type="button"
                        onClick={() => onSelectPath(path, kind, status)}
                      >
                        {path}
                      </button>
                    ) : (
                  ...
                  <td>{String(item.kind ?? item.status ?? "-")}</td>
                  <td>{item.sizeBytes === undefined ? "-" : formatBytes(item.sizeBytes)}</td>
                  <td>
                    {item.modifiedMs
                      ? new Date(Number(item.modifiedMs)).toLocaleString()
  ...
  return (
    <>
      <div className="management-actions">
        <Action disabled={busy} label={t("listHosts")} onClick={() => run("ssh.hosts.list")} />
        <Action
          disabled={busy}
          label={t("clientStatus")}
          onClick={() => run("ssh.client_status")}
        />
      </div>
      <div className="management-grid">
        ...
        <label className="checkbox-row">
          <input
            type="checkbox"
            checked={field("acceptNewHostKey") === "true"}
            onChange={(event) => update("acceptNewHostKey", String(event.target.checked))}
          />
          {t("acceptNewHostKey")}
        </label>
      </div>
      <div className="management-actions">
        <Action
          ...
          onClick={() =>
            run(
              "ssh.test_connection",
              { hostId: field("hostId"), acceptNewHostKey: field("acceptNewHostKey") === "true" },
              field("acceptNewHostKey") === "true",
            )
          }
        />
        <Action
          disabled={busy}
          label={t("checkPath")}
          onClick={() => run("ssh.check_path", { hostId: field("hostId"), path: field("path") })}
        />
        <Action
          disabled={busy}
          label={t("listDirectories")}
          onClick={() =>
            run("ssh.list_directories", { hostId: field("hostId"), path: field("path") || "/" })
          }
        />
      </div>
      <div className="management-grid">
        ...
        <Input
          label={t("username")}
          value={field("sshUsername")}
          onChange={(value) => update("sshUsername", value)}
        />
      </div>
      <div className="management-actions">
        <Action
        ...
        <Action
          disabled={busy}
          label={t("deleteHost")}
          onClick={() => run("ssh.host.delete", { hostId: field("hostId") }, true)}
        />
      </div>
    </>
  );
  ...
  return (
    <>
      ...
      <div className="management-grid">
        ...
        <Input
          label={t("targetPath")}
          value={field("targetPath")}
          onChange={(value) => update("targetPath", value)}
        />
      </div>
      <div className="management-actions">
        <Action
          ...
          label={t("listFiles")}
          onClick={() => run("file.list", { path: field("path") })}
        />
        <Action
          disabled={busy || disabled}
          label={t("searchFiles")}
          onClick={() => run("file.search", { query: field("query") })}
        />
        <Action
          disabled={busy || disabled}
          label={t("searchContent")}
          onClick={() => run("file.search_content", { query: field("query") })}
        />
        <Action
          disabled={busy || disabled}
          label={t("readText")}
          onClick={() => run("file.read_text", { path: field("path") })}
        />
        <Action
          disabled={busy || disabled}
          label={t("readImage")}
          onClick={() => run("file.read_image", { path: field("path") })}
        />
      </div>
      <div className="management-actions">
        <Action
          ...
          label={t("createFile")}
          onClick={() =>
            run("file.create", { parentPath: field("path"), name: field("name") }, true)
          }
        />
        <Action
          disabled={busy || disabled}
          label={t("createDirectory")}
          onClick={() =>
            run("file.create_directory", { parentPath: field("path"), name: field("name") }, true)
          }
        />
        <Action
          disabled={busy || disabled}
          label={t("rename")}
          onClick={() => run("file.rename", { path: field("path"), name: field("name") }, true)}
        />
        <Action
          disabled={busy || disabled}
          label={t("copy")}
          onClick={() =>
            run(
              "file.copy",
              {
                ...
                targetParentPath: field("targetPath"),
                name: field("name"),
              },
              true,
            )
        ...
        <Action
          ...
          onClick={() => run("file.delete", { path: field("path") }, true)}
        />
      </div>
    </>
  );
    ...
    <>
      ...
      <div className="management-actions">
        <Action
          disabled={busy || disabled}
          label={t("gitStatus")}
          onClick={() => run("git.status")}
        />
        <Action
          disabled={busy || disabled}
          label={t("branches")}
          onClick={() => run("git.branches")}
        />
        <Action
          disabled={busy || disabled}
          label={t("viewDiff")}
          onClick={() =>
            run("git.diff", {
              path: field("path"),
              status: field("status") || "M",
              whitespace: field("whitespace") || "exact",
              contextLines: Number(field("contextLines") || 3),
            })
          }
        />
        <Action
      ...
      <div className="management-grid">
        ...
        />
        <Input
          label={t("pathsCommaSeparated")}
          value={field("paths")}
          onChange={(value) => update("paths", value)}
        />
        <Input
          label={t("relativePath")}
          value={field("path")}
        ...
        />
        <label>
          <span>{t("diffWhitespace")}</span>
          <select
            value={field("whitespace") || "exact"}
            onChange={(event) => update("whitespace", event.target.value)}
          >
            <option value="exact">{t("diffExact")}</option>
            <option value="ignore-eol">{t("diffIgnoreEol")}</option>
            <option value="ignore-all">{t("diffIgnoreAll")}</option>
          </select>
        </label>
        <label>
          <span>{t("diffContext")}</span>
          <select
            value={field("contextLines") || "3"}
            onChange={(event) => update("contextLines", event.target.value)}
          >
      ...
      <div className="management-actions">
        <Action
          disabled={busy || disabled}
          label={t("checkout")}
          onClick={() => run("git.checkout", { branch: field("branch"), remote: false }, true)}
        />
        <Action
          disabled={busy || disabled}
          label={t("createBranch")}
          onClick={() => run("git.create_branch", { branch: field("branch") }, true)}
        />
        <Action
          disabled={busy || disabled}
          label={t("stage")}
          onClick={() => run("git.stage", { paths: csv(field("paths")) }, true)}
        />
        <Action
          disabled={busy || disabled}
          label={t("unstage")}
          onClick={() => run("git.unstage", { paths: csv(field("paths")) }, true)}
        />
        <Action
          disabled={busy || disabled}
          label={t("commit")}
          onClick={() => run("git.commit", { message: field("message") }, true)}
        />
        <Action
          disabled={busy || disabled}
          label={t("pull")}
          onClick={() => run("git.pull", { strategy: field("pullStrategy") }, true)}
        />
        <Action
          disabled={busy || disabled}
          label={t("push")}
          onClick={() => run("git.push", {}, true)}
        />
        <Action
          disabled={busy || disabled}
          label={t("discard")}
          onClick={() =>
            run(
              "git.discard",
              {
                items: csv(field("paths")).map((path) => ({
                  path,
                  status: field("status") || "M",
                })),
              },
        ...
        <Action
          disabled={busy || disabled}
          label={t("deleteUntracked")}
          onClick={() => run("git.delete_untracked", { paths: csv(field("paths")) }, true)}
        />
      </div>
    </>
  );
  ...
  return (
    <>
      ...
      <div className="management-grid">
        ...
        <Input
          label={t("worktreeId")}
          value={field("worktreeId")}
          onChange={(value) => update("worktreeId", value)}
        />
        <label className="checkbox-row">
          <input
            type="checkbox"
      ...
      <div className="management-actions">
        <Action
          disabled={busy || disabled}
          label={t("listWorktrees")}
          onClick={() => run("worktree.list")}
        />
        <Action
          disabled={busy || disabled}
          label={t("createWorktree")}
          onClick={() => run("worktree.create", { taskName: field("taskName") }, true)}
        />
        <Action
          disabled={busy || disabled}
          label={t("checkDeps")}
          onClick={() => run("worktree.check_deps", { worktreeId: field("worktreeId") })}
        />
        <Action
          disabled={busy || disabled}
          label={t("mergeWorktree")}
          onClick={() => run("worktree.merge", { worktreeId: field("worktreeId") }, true)}
        />
        <Action
          disabled={busy || disabled}
          label={t("removeWorktree")}
          onClick={() =>
            run("worktree.remove", { worktreeId: field("worktreeId"), deleteBranch }, true)
          }
        />
      </div>
    </>
...
function HookControls({ t, field, update, run, busy }: ControlProps) {
  return (
    <>
      <label>
        <span>{t("hookTarget")}</span>
        <select value={field("target")} onChange={(event) => update("target", event.target.value)}>
      ...
      <div className="management-actions">
        <Action
          disabled={busy}
          label={t("hookStatus")}
          onClick={() => run("hook.status", { target: field("target") })}
        />
        <Action
          disabled={busy}
          label={t("install")}
          onClick={() => run("hook.install", { target: field("target") }, true)}
        />
        <Action
          disabled={busy}
          label={t("repair")}
          onClick={() => run("hook.repair", { target: field("target") }, true)}
        />
        <Action
          disabled={busy}
          label={t("testAction")}
          onClick={() => run("hook.test", { target: field("target") })}
        />
        <Action
          disabled={busy}
          label={t("uninstall")}
          onClick={() => run("hook.uninstall", { target: field("target") }, true)}
        />
      </div>
    </>
  );
