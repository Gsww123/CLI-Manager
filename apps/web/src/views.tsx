    ...
    <main className="host-home">
      <header className="host-home-header">
        <div className="host-brand">
          <AppLogo />
          <div>
            <strong>CLI-Manager</strong>
            <span>{props.t("hostsSubtitle")}</span>
          </div>
        </div>
        <div className="host-home-actions">
          <span className={`browser-status ${props.socketState}`}>
            {props.socketState === "open" ? <Wifi size={16} /> : <WifiOff size={16} />}
            {props.t(
          ...
          <button
            ...
            aria-label={props.t("theme")}
          >
            {props.resolvedTheme === "dark" ? <Moon size={19} /> : <Sun size={19} />}
          </button>
          <button
            className="host-account"
            type="button"
            onClick={props.onLogout}
            aria-label={props.t("logout")}
          >
      ...
      <section className="host-home-content" aria-label={props.t("hostsTitle")}>
        {devices.length === 0 ? (
          <div className="host-empty">
            <Monitor size={42} />
            <h2>{props.t("noHostsTitle")}</h2>
            <p>{props.t("noHostsHint")}</p>
            <button className="primary-button" type="button" onClick={() => setPairingOpen(true)}>
              <Plus size={18} />
              {props.t("pairDevice")}
            </button>
          </div>
          ...
          <>
            <section className="host-inventory" aria-labelledby="device-inventory-title">
              <div className="host-inventory-toolbar">
                ...
                <div className="host-inventory-controls">
                  <div className="host-search">
                    <label className="sr-only" htmlFor="host-device-search">
                      {props.t("searchDevices")}
                    </label>
                    <Search size={17} />
                    <input
                      id="host-device-search"
                      type="search"
                      value={query}
                      onChange={(event) => setQuery(event.target.value)}
                      placeholder={props.t("searchDevices")}
                    />
                    {query && (
                      <button
                        type="button"
                  ...
                  {!props.restricted && (
                    <button
                      className="secondary-button host-pair-button"
                      type="button"
              ...
              {visibleDevices.length === 0 ? (
                <div className="host-filter-empty">
                  <Search size={32} />
                  <h3>{props.t("noMatchingDevices")}</h3>
                  <p>{props.t("noMatchingDevicesHint")}</p>
                  <button className="secondary-button" type="button" onClick={clearFilters}>
                    {props.t("clearFilters")}
                  </button>
                </div>
              ) : (
                <div
                  className="host-list"
                  aria-label={`${props.t("hostsTitle")} ? ${visibleDevices.length}`}
                >
                  {visibleDevices.map((device) => (
                    <div className="host-card-wrap" key={device.id}>
                      ...
                      >
                        <span
                          className={`host-card-visual${device.wallpaperRevision ? " has-wallpaper" : ""}`}
                        >
                          {device.wallpaperRevision && (
                            <img
                              className="host-card-wallpaper"
                              src={deviceWallpaperUrl(device) ?? undefined}
                              alt=""
                              loading="lazy"
                            />
                          )}
                        ...
                        <span className="host-card-body">
                          <span className="host-card-heading">
                            <strong className="host-card-name">
                              {device.name}
                              {clientKindLabel(device, props.t)
                                ? ` · ${clientKindLabel(device, props.t)}`
                                : ""}
                            </strong>
                            <span className={`host-status ${device.status}`}>
                              <span
                                className={`status-dot ${device.status === "online" ? "" : "warning"}`}
                              />
                              {props.t(device.status === "online" ? "online" : "offline")}
                            </span>
                          </span>
                          <span className="host-facts">
                            <span className="host-fact">
                              <span>{props.t("hostName")}</span>
                              <strong>{device.hostInfo?.hostName || props.t("unknown")}</strong>
                            </span>
                            <span className="host-fact">
                              <span>{props.t("system")}</span>
                              <strong>{device.hostInfo?.osVersion || props.t("unknown")}</strong>
                            </span>
                            <span className="host-fact">
                              <span>{props.t("platform")}</span>
                              <strong>{device.platform || props.t("unknown")}</strong>
                            </span>
                            <span className="host-fact">
                              <span>{props.t("appVersion")}</span>
                              <strong className="host-version-badge">
                                {device.appVersion ? `v${device.appVersion}` : props.t("unknown")}
                              </strong>
                            </span>
                          </span>
                        </span>
                        <span className="host-card-side">
                          <span className="host-heartbeat">
                            <Clock3 size={14} />
                            <span>{props.t("lastHeartbeat")}</span>
                            <time dateTime={formatServerDateTime(device.lastSeenAt)}>
                              {device.lastSeenAt === null
                                ? props.t("noHeartbeat")
                                : formatServerTime(device.lastSeenAt)}
                            </time>
                          </span>
                          <span className="host-card-open">
                            {props.t("openHost")}
                      ...
                      {!props.restricted && (
                        <span className="host-card-actions">
                          <button
                            ...
                            type="button"
                            aria-label={props.t("mobileQr")}
                            onClick={() => setMobileDeviceId(device.id)}
                          >
                            <Wifi size={17} />
                          </button>
                          <button
      ...
      {pendingRemoval && (
        <div
          ...
          role="presentation"
          onMouseDown={(event) => {
            if (event.target === event.currentTarget && !removingDeviceId)
        ...
        >
          ...
          >
            ...
            <h2 id="remove-device-title">{props.t("removeDevice")}</h2>
            <p>{props.t("removeDeviceConfirmation").replace("{name}", pendingRemoval.name)}</p>
            <div className="confirm-dialog-actions">
              ...
              <button
                ...
                disabled={Boolean(removingDeviceId)}
              >
                {removingDeviceId && <LoaderCircle className="spin" size={16} />}
                {props.t("removeDevice")}
              </button>
            </div>
          </div>
        </div>
      ...
      {mobileDeviceId && (
        <OverlayPanel
          title={props.t("mobileQr")}
          closeLabel={props.t("close")}
          onClose={() => setMobileDeviceId(undefined)}
        >
          <MobileQr t={props.t} deviceId={mobileDeviceId} />
        </OverlayPanel>
      )}
      {pairingOpen && (
        <OverlayPanel
          title={props.t("pairDevice")}
          closeLabel={props.t("close")}
          onClose={closePairing}
        >
          <PairingForm t={props.t} state={props.pairing} onClaim={props.onClaimPairing} />
        </OverlayPanel>
      )}
    </main>
  );
  ...
  return (
    ...
    >
      ...
      <main className="main-panel" id="conversation-main">
        <header className="desktop-header">
          <div className="context-block">
            ...
            <div className="context-meta">
              <span>
                {t("worktree")}: {selectedProjectContext?.projectName ?? t("unknown")}
              </span>
              <span>
                CLI: {selectedProjectContext?.source ?? capabilityValue(selectedDevice, "cli:")}
              </span>
              <span>
                {t("terminal")}: {terminalStatusLabel(t, props.terminalStatus)}
              </span>
            </div>
          </div>
          <div className="header-actions">
            <button
              className="icon-button"
              type="button"
              onClick={props.onBackToHosts}
              aria-label={t("backToHosts")}
            >
              <ArrowLeft size={20} />
            </button>
            <button
            ...
            <button
              ...
              aria-label={t("theme")}
            >
              {props.resolvedTheme === "dark" ? <Moon size={20} /> : <Sun size={20} />}
            </button>
          </div>
        </header>
        <header className="mobile-header">
          <button
            className="icon-button mobile-projects-toggle"
            type="button"
            onClick={() => {
              projectDrawerTabs.current = props.terminalTabs
          ...
          <div>
            <strong>{selectedSession?.title ?? "CLI-Manager"}</strong>
            <span className="mobile-device-line">
              <span
                className={`status-dot ${selectedDevice?.status === "online" ? "" : "warning"}`}
                role="img"
                aria-label={selectedDevice?.status === "online" ? t("online") : t("offline")}
              />
              {selectedDevice?.name ?? t("noDevice")}
            </span>
          </div>
          <button
            className="icon-button"
        ...
        <section className="workspace-content terminal-workspace">
          <div className="source-banner" role="status" aria-live="polite">
            <span
              className={`source-badge ${props.terminalStatus === "running" ? "live" : selectedFreshness}`}
            >
              {terminalStatusLabel(t, props.terminalStatus)}
            </span>
            <span>
              {props.terminalSessionId
                ? `${t("terminalSession")}: ${props.terminalSessionId}`
                : t("terminalNotStarted")}
            </span>
            {props.terminalSessionId && (
              <span className="terminal-project-meta">
                {t("project")}: {activeTerminalContext?.projectName ?? t("unknown")}
              </span>
            )}
            {props.terminalSessionId && (
              <span className="terminal-cwd" title={activeTerminalContext?.cwd ?? undefined}>
                {t("workingDirectory")}: {activeTerminalContext?.cwd ?? t("unknown")}
              </span>
            )}
            {props.terminalSessionId && (
              <span>
                {t(
                  props.terminalControlMode === "web"
                    ? "terminalControlWeb"
                    : "terminalControlDesktop",
                )}
              </span>
            )}
                ...
                return (
                  <div className={`terminal-tab${active ? " active" : ""}`} key={tab.sessionId}>
                    ...
                    <button
                      ...
                      type="button"
                      onClick={() => props.onCloseTerminal(tab.sessionId)}
          ...
          {props.terminalTabs.length > 0 && (
            <div ref={fileLayout.stackRef} className={`web-terminal-stack${filesDocked ? " has-files-dock" : ""}`}>
              ...
              {props.terminalTabs.map((tab) => {
                ...
                return (
                  <div
                    className={`web-terminal-frame${active ? " active" : ""}`}
                    key={tab.sessionId}
                    role="tabpanel"
                    aria-hidden={!active}
                    inert={!active}
                  >
                    <WebTerminal
                      ...
                      errorLabel={t("terminalRenderError")}
                    ...
                    />
                    <SubagentPanel
                      agents={(props.workspace?.subagents ?? []).filter(
                        (agent) => agent.parentSessionId === tab.sessionId,
                      )}
                      active={active}
                      t={t}
                    />
      ...
      >
        ...
        {selectedDevice ? (
          <DeviceCard device={selectedDevice} t={t} syncText={syncText} />
        ) : (
          <p className="muted">{t("noDeviceHint")}</p>
        )}
        {!props.restricted && (
          <button className="secondary-button" type="button" onClick={() => setPairingOpen(true)}>
            <Plus size={18} />
            {t("pairDevice")}
          </button>
        )}
        ...
        <div className="capability-list">
          {selectedDevice?.capabilities.length ? (
            selectedDevice.capabilities.map((item) => <span key={item}>{item}</span>)
          ) : (
            <span>{t("unknown")}</span>
          )}
        </div>
      ...
      <nav className="bottom-nav" aria-label={t("workbench")}>
        ...
        <button
          type="button"
          onClick={props.restricted ? props.onBackToHosts : () => setPairingOpen(true)}
        >
          <Monitor size={23} />
          <span>{t("devices")}</span>
        </button>
      ...
      {projectsOpen && (
        <OverlayPanel title={t("projects")} closeLabel={t("close")} onClose={closeProjects}>
          <ProjectSidebar
            {...props}
            mobile
            onPair={() => {
            ...
            onOpenTerminal={() => {
              ...
              if (props.terminalTabs.some((tab) => tab.contextKey === selectedProjectContext?.key))
                closeProjects();
            }}
            onOpenPanel={(panel, contextKey) => {
              props.onSelectProjectContext(contextKey);
              ...
              if (panel === "history") {
                setHistoryContext(
                  props.projectContexts.find((context) => context.key === contextKey),
                );
                setHistoryOpen(true);
              } else {
                openFiles(props.projectContexts.find((context) => context.key === contextKey));
            ...
            items={
              historyContext
                ? props.history.filter(
                    (session) =>
                      session.source === historyContext.source &&
                      (historyContext.projectId
                        ? session.projectId === historyContext.projectId
                        : session.projectKey === historyContext.projectKey) &&
                      (session.worktreeId ?? null) === (historyContext.worktreeId ?? null),
                  )
                : props.history
            }
...
function ProjectSidebar(props: WorkbenchProps & { mobile?: boolean; onPair: () => void; onOpenPanel: (panel: "file" | "history", contextKey: string) => void }) {
  return (
    ...
    >
      {props.mobile ? (
        <DeviceLine device={props.selectedDevice} t={props.t} socketState={props.socketState} />
      ) : (
      ...
      <button
        ...
        disabled={
          !props.selectedProjectContext ||
          props.selectedDevice?.status !== "online" ||
          (props.mobile && (props.sending || props.socketState !== "open"))
        }
      >
        <Plus size={18} />
        <span>{props.t("newTerminal")}</span>
      </button>
      {props.mobile && (
      ...
      <div className="project-tree">
        <div className="side-section-title">
          ...
          <span className="count">{props.workspace?.projects.length ?? 0}</span>
        </div>
        <ProjectTree
          t={props.t}
          ...
          dragEnabled={Boolean(
            !props.mobile &&
            props.selectedDevice?.status === "online" &&
            props.selectedDevice.capabilities.includes("project.management"),
          )}
          managementEnabled={Boolean(
            props.selectedDevice?.status === "online" &&
            (!props.mobile || props.socketState === "open") &&
            props.selectedDevice.capabilities.includes("project.management"),
          )}
          defaultCollapsed
      ...
      <div className="sidebar-footer">
        {!props.restricted && (
          <button className="footer-row" type="button" onClick={props.onPair}>
            <Monitor size={20} />
            <span>{props.t("pairDevice")}</span>
          </button>
        )}
        <button className="account-row" type="button" onClick={props.onLogout}>
          <span className="avatar">{props.userName.slice(0, 1).toUpperCase()}</span>
          <span>{props.userName}</span>
          <LogOut size={16} />
        </button>
...
function HistoryList({ t, items, selectedId, onSelect }: { t: T; items: HistorySessionSummary[]; selectedId?: string; onSelect: (id: string) => void }) {
  return (
    <div className="history-list">
      ...
      {items.length === 0 ? (
        <p className="empty-copy">{t("noHistory")}</p>
      ) : (
        items.map((session) => (
          <button
            className={`history-row${selectedId === session.sessionId ? " active" : ""}`}
            type="button"
          ...
          >
            ...
            <span>
              ...
              <small>
                {session.projectKey} ? {formatServerTime(session.updatedAt)}
              </small>
            </span>
            <span
              className={`freshness-dot ${session.freshness}`}
              title={t(
    ...
    <form className="pairing-form" onSubmit={submit}>
      ...
      {state.status === "error" && (
        <p className="form-error" role="alert">
          {pairingError(t, state.code, state.message)}
        </p>
      )}
      <button
        className="primary-button"
        type="submit"
        disabled={state.status === "submitting" || !code.trim()}
      >
  ...
  return (
    <>
      <div className="welcome-block">
        <AppLogo />
        <h1>{t("greeting")}</h1>
        <p>{t("greetingQuestion")}</p>
      </div>
      <div className="suggestion-grid">
        {(
          [
            ["solveProject", "promptSolve", Folder],
            ["fixIssue", "promptFix", Shield],
            ["brainstorm", "promptBrainstorm", Bot],
          ] as const
          ...
          >
            ...
            <span>
              ...
              <small>{device.status === "online" ? t("ready") : t("offlineDraftOnly")}</small>
            </span>
          </button>
        ))}
      </div>
...
function ConversationTimeline({ t, items }: { t: T; items: TimelineItem[] }) {
  return (
    <section className="timeline" aria-label={t("conversationTimeline")} aria-live="polite">
      {items.map((item) =>
        item.type === "operation" ? (
          <OperationCard key={item.id} t={t} item={item} />
        ) : (
          <article className={`timeline-item ${item.type}-item`} key={item.id}>
            <header>
              {item.type === "prompt" ? <MessageCircle size={18} /> : <Bot size={18} />}
              <strong>
                {t(
                  item.type === "prompt"
                    ? "you"
              ...
              <time>{formatServerTime(item.occurredAt)}</time>
            </header>
            <p>{item.text}</p>
            {item.type === "assistant" && item.streaming && (
              <small>{t("assistantStreaming")}</small>
            )}
          </article>
        ),
      )}
...
function OperationCard({ t, item }: { t: T; item: Extract<TimelineItem, { type: "operation" }> }) {
  ...
  return (
    <article className={`timeline-item operation-item ${operation.status}`}>
      <header>
      ...
      <div className="operation-grid">
        <span>{t("operationKind")}</span>
        <strong>{operation.kind}</strong>
        <span>{t("operationStatus")}</span>
        <strong>{operationStatusLabel(t, operation.status)}</strong>
      </div>
      {operation.error && (
        <>
          <p className="form-error" role="alert">
            {errorKey ? t(errorKey) : localizedError(t, operation.error.code)}
          </p>
          <details>
      ...
      <small>
        {t("updatedAt")} {formatServerTime(operation.updatedAt)}
      </small>
    </article>
  );
}
  ...
  return (
    <form className="composer" onSubmit={submit}>
      ...
      <textarea
        ...
        placeholder={offline ? t("offlineComposerPlaceholder") : t("composerPlaceholder")}
        rows={3}
      />
      <div className="composer-toolbar">
        <span className={`composer-state ${offline ? "offline" : "online"}`}>
          {offline ? <WifiOff size={17} /> : <Radio size={17} />}
          {t(offline ? "offlineDraftOnly" : "ready")}
        </span>
        <button className="send-button" type="submit" aria-label={t("send")} disabled={disabled}>
          <Send size={20} />
        </button>
      </div>
      {message && (
        <p className="composer-feedback" role="alert">
          {localizedError(t, message)}
        </p>
      )}
    </form>
  );
