...
- `extensions_mcp_native_preview(cli) -> { cli, path, fingerprint, existingKeys, enabledKeys, removedKeys, changed }`.
...
- Settings close, navigation away from extensions and MCP/Skills switching share a three-way guard: apply-and-continue (only on full success), leave without applying (retain definitions/pending state), or stay. In-flight operations prevent navigation.
